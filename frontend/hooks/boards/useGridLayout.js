...
import { mapToLayoutItems, snapToStep, Y_SNAP_STEP } from '../../components/layout/Grid/gridUtils';

// Layout state machine for the board grid: tracks item positions, in-progress
// drag/resize previews, collision detection, snapping, and the derived
// container height. The component renders from the values this returns.
export default function useGridLayout({
    ...
    const getGridPositionFromPixels = useCallback((pixelX, pixelY) => {
        ...
        return {
            ...
            snappedY: snapToStep(rawY, Y_SNAP_STEP)
        };
    ...
    const snappedY = Math.max(0, snapToStep(clampedRawY, Y_SNAP_STEP));
    ...
    const handleDragEnd = useCallback((id) => {
        ...
        const targetY = Math.max(0, snapToStep(preview.snappedY ?? preview.y, Y_SNAP_STEP));
    ...
    }, [hasCollision, onLayoutChange, updateLayoutState, cols]);
    ...
    const handleResizeStart = useCallback((id, direction) => {
        ...
        const heightStep = constraints.heightStep > 0 ? constraints.heightStep : 1;
        const minHeightFloor = Math.min(1, heightStep);
        const minWidth = Math.max(constraints.minWidth ?? item.w, 1);
        const minHeight = Math.max(constraints.minHeight ?? item.h, minHeightFloor);
        ...
        resizeStateRef.current = {
            ...
            constraints: { minWidth, minHeight, maxWidth, maxHeight, heightStep }
        };
    ...
    const handleResizeMove = useCallback((id, direction, dx, dy) => {
        ...
        let snappedH = Math.max(constraints.minHeight, snapToStep(rawHeight, heightStep));
    ...
    const handleResizeEnd = useCallback((id) => {
        ...
        const targetHeight = Math.max(constraints.minHeight, Math.min(preview.snappedH ?? snapToStep(preview.h, heightStep), constraints.maxHeight));
    ...
    const previewHighlightPosition = useMemo(() => {
        if (dragPreview) {
            ...
            const clampedY = Math.max(0, dragPreview.snappedY ?? snapToStep(dragPreview.y, Y_SNAP_STEP));
        ...
        if (resizePreview) {
            ...
            const sourceItem = itemMap.get(resizePreview.id);
            const heightStep = sourceItem?.resizeConstraints?.heightStep > 0
                ? sourceItem.resizeConstraints.heightStep
                : 1;
            const minHeight = Math.min(1, heightStep);
            const width = Math.max(1, resizePreview.snappedW ?? Math.round(resizePreview.w));
            const height = Math.max(minHeight, resizePreview.snappedH ?? snapToStep(resizePreview.h, heightStep));

            return getItemPosition({ ...layoutItem, w: width, h: height });
        }

        return null;
    }, [dragPreview, resizePreview, layout, cols, getItemPosition, itemMap]);
